#!/bin/bash
# Removes an agentmux Claude Code macOS LaunchAgent instance. Does not kill a
# currently running tmux session or touch the instance's workdir.
set -euo pipefail

usage() {
    cat <<'EOF'
Removes an agentmux Claude Code macOS LaunchAgent instance.
...
echo "Removed agentmux claude-code instance $INSTANCE_NAME LaunchAgents. Tmux session (if running) was left alone."
