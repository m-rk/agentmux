#!/bin/bash
# Removes one agentmux Linux systemd instance. Does not kill a currently
# running tmux session or delete the instance workdir.
set -euo pipefail

usage() {
    cat <<'EOF'
Removes one agentmux Linux systemd instance.
...
echo "Removed agentmux instance $INSTANCE_NAME systemd units. Tmux session and workdir were left alone."
