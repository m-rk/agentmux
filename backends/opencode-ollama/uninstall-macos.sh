#!/bin/bash
# Removes the agentmux opencode + Ollama Cloud macOS LaunchAgents. Does not
# kill a currently running tmux session, touch ~/.agentmux/opencode-ollama,
# or touch the ollama installation/service.
set -euo pipefail
...
echo "Removed agentmux opencode-ollama LaunchAgents. Tmux session (if running) and ollama were left alone."
