...
| any (fallback) | `ViewPane` + discovery status | running/idle transitions, pane unchanged while `running`, visible menus |
...
- Quiet hours are optional; alerts held during them go into the digest.
...
| **Dedup/merge** | Noul "same underlying issue as open alert X?" | Stops one outage from causing one alert per instance |
...
4. **Jev live, plus the remaining collectors.** Jev gates start controlling
   ...
   others, and a TUI badge for instances with open signals.

## Open questions

- The TypeSafe item UUID in 1Password.
- Whether digests and alerts use the same webhook or separate channels.
- The digest time, and whether you want quiet hours.
